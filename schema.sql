...
CREATE TABLE events (
    ...
    removed         TINYINT(1)          NOT NULL DEFAULT 0,
    -- Когда объект пропал из фида (ставит bridge.py --sweep): status=3 + 24 ч (ключ Redis истёк
    -- по TTL) либо NOW() при аномалии (ключа активного события нет в Redis). removed=1 при этом.
    -- Строки с gone_at старше retention_days удаляются сборщиком (маркеты/исходы — каскадом).
    ...
    KEY         idx_sweep       (status, removed, uts),     -- кандидаты правила 24 ч
    KEY         idx_gone        (removed, gone_at),         -- кандидаты ретенции
