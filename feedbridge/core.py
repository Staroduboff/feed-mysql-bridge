...
tx_mark()/tx_undo_to(n) делают то же для отката к точке сохранения (SAVEPOINT).

Второй источник фантомных id — чужое удаление. Сборщик (bridge.py --sweep)
отдельным процессом удаляет события по ретенции, маркеты и исходы уходят за ними
каскадом, а кэши работающего листенера об этом не знают. drop_id_caches()
сбрасывает кэши событий и маркетов: листенер зовёт его при отказе внешнего
ключа (1452) и по таймеру (инцидент 24.09.2026: четыре часа requeue одного пакета).
...
tx_mark() / tx_undo_to(n)       То же для отката к SAVEPOINT.
drop_id_caches()                Сбросить кэши id событий и маркетов (перечитаются из MySQL).
...
class Bridge:
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self.gate = Gate(cfg)
        # Потерянная дельта — это цена на экране игрока, которая могла остаться неверной,
        # поэтому публикатор сообщает о потерях гейту, и тот закрывает приём ставок.
        self.pub.on_drop = self.gate.note_drop
    ...
    def tx_mark(self) -> int:
        """Позиция в журнале пометок — запомнить перед SAVEPOINT."""
        return len(self._tx_marks)

    def tx_undo_to(self, mark: int) -> None:
        """Откат к SAVEPOINT: снять записи кэшей, сделанные после позиции mark."""
        for cache, key in self._tx_marks[mark:]:
            cache.pop(key, None)
        del self._tx_marks[mark:]

    def drop_id_caches(self) -> None:
        """Сбросить кэши id событий и маркетов: строки могли удалить в обход листенера.

        Справочники (виды спорта, категории, турниры) сборщик не удаляет, их кэши живут.
        """
        self.ev_cache.clear()
        self.mkt_cache.clear()
        self._tx_marks = [(c, k) for c, k in self._tx_marks
                          if c is not self.ev_cache and c is not self.mkt_cache]
