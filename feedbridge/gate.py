...
qlag_max, lag_max
       Максимумы qlag и lag за `peak_window` секунд. Нужны витрине здоровья: она
       собирается раз в минуту и берёт мгновенный снимок, поэтому всплеск в пару
       секунд между сборами не увидела бы вовсе. Гейт тикает каждую секунду и
       запоминает пик за окно — минутный сбор забирает уже его.
...
pdrop  Секунд с последней дельты, НЕ доставленной в браузеры (см. publish.py).
       Отличается от остальных сигналов природой: те говорят «данные устарели»,
       а этот — «данные разошлись». Цена на экране игрока могла остаться прежней,
       хотя в базе она уже другая, и ставка по ней — прямой убыток. Поэтому любая
       потеря закрывает приём на `pub_drop_window` секунд плюс обычная выдержка.
...
THR = dict(
    ...
    pub_drop_window=60, # столько секунд после потери дельты приём закрыт
    peak_window=120,    # за сколько секунд помнить пики qlag и lag для витрины
...
REASONS = {
    ...
    "pubdrop": "дельты не доставлены в браузеры",
...
class Gate:
    ...
    def __init__(self, cfg):
        ...
        self._drop_at = None        # время последней потери дельт (publish.py)
        self._drops = 0             # сколько дельт потеряно за сессию
        self._peaks = collections.deque(maxlen=1024)   # (время, qlag, lag) для пиков
    ...
    def note_drop(self, n=1, now=None):
        """Публикатор не доставил n дельт в браузеры. Вызывается из publish.py.

        now — для тестов, которые подают время явно; в бою берётся системное.
        """
        self._drops += int(n or 0)
        self._drop_at = now or time.time()
    ...
    def tick(self, now=None):
        ...
        m = self._measure(now)
        # пики за окно — единственный способ показать секундный всплеск на витрине,
        # которая собирается раз в минуту
        self._peaks.append((now, m.get("qlag"), m.get("lag")))
        edge = now - self.thr["peak_window"]
        while self._peaks and self._peaks[0][0] < edge:
            self._peaks.popleft()
        qs = [p[1] for p in self._peaks if p[1] is not None]
        ls = [p[2] for p in self._peaks if p[2] is not None]
        m["qlag_max"] = max(qs) if qs else None
        m["lag_max"] = max(ls) if ls else None
        self.metrics = m
        bad = self._bad(m)
    ...
    def _measure(self, now):
        ...
        m = dict(lag=None, sil=None, qlag=None, hb=None, pdrop=None, pdrops=self._drops, ready=ready,
        ...
        if self._drop_at:
            m["pdrop"] = max(0.0, round(now - self._drop_at, 1))
    ...
    def _bad(self, m):
        ...
        if m["hb"] is not None and m["hb"] >= t["hb_crit"]:
            bad.append("hb")
        if m["pdrop"] is not None and m["pdrop"] <= t["pub_drop_window"]:
            bad.append("pubdrop")
        if m["srv_active"] == 0:
            bad.append("inactive")
        if m["srv_amqp"] == 0:
