...
--------------------
many(statement, n) возвращает тот же оператор с кортежем VALUES, повторённым n
раз: одна отправка вместо n. Своя сборка, а не executemany PyMySQL, потому что
его склейка в один INSERT срабатывает только когда кортеж VALUES состоит
исключительно из плейсхолдеров; у SQL_MKT и SQL_OC там есть литералы (NOW(),
NULL), и executemany молча деградировал бы до построчной отправки — то есть до
одного обращения к MySQL на строку.
"""
...
# ── многострочная сборка операторов ───────────────────────────────────────────

# Разбор оператора выполняется один раз на константу и кэшируется: горячий путь
# листенера собирает многострочный INSERT на каждый чанк.
_MANY_CACHE: dict = {}


def _split_values(statement: str) -> tuple:
    """Разбить INSERT на «шапку», кортеж VALUES и «хвост» (ON DUPLICATE KEY UPDATE).

    Скобки считаются с учётом вложенности: в кортеже встречаются вызовы функций
    (NOW()), поэтому наивный поиск первой закрывающей скобки не подходит.
    """
    head_end = statement.upper().index("VALUES")
    open_at  = statement.index("(", head_end)
    depth    = 0
    for i in range(open_at, len(statement)):
        ...
        if ch == "(":
            ...
            depth -= 1
            if depth == 0:
                return statement[:open_at], statement[open_at:i + 1], statement[i + 1:]
    raise ValueError("в операторе не найден закрытый кортеж VALUES")


def many(statement: str, n: int) -> str:
    """Многострочная версия оператора: кортеж VALUES повторён n раз.

    Параметры передаются одним плоским списком в том же порядке, что и строки.
    """
    if n <= 1:
        return statement
    parts = _MANY_CACHE.get(statement)
    if parts is None:
        parts = _MANY_CACHE[statement] = _split_values(statement)
    head, tpl, tail = parts
    return head + ", ".join([tpl] * n) + tail
