...
брокеру — групповое (basic_ack multiple) и только после commit; при ТРАНЗИЕНТНОМ
сбое БД (обрыв, failover, дедлок) весь пакет уходит в requeue (basic_nack
multiple), ни одно сообщение не теряется.

Ошибка в ДАННЫХ в requeue не уходит: тот же пакет вернулся бы с той же ошибкой, и
мост крутил бы его по кругу, пока сообщения не истекут по TTL очереди (так было
24.09.2026: 4 ч простоя). Вместо этого:
  1. отказ внешнего ключа (1452) почти всегда означает устаревший id в кэше —
     сборщик удалил событие, а кэш о нём помнит. Кэши id событий и маркетов
     сбрасываются, пакет переигрывается один раз, родители ищутся заново;
  2. если пакет снова не записался — он пишется по одному объекту, каждый под
     своим SAVEPOINT: битые объекты откатываются и считаются в apply_error,
     остальные фиксируются, пакет подтверждается брокеру.
Кроме того, кэши id событий и маркетов сбрасываются раз в _ID_CACHE_RESET_S.
...
# Отказ внешнего ключа: родителя с таким id в базе нет. Для моста это признак
# устаревшего id в кэше (строку удалил сборщик), а не битых данных фида.
_FK_FAIL_CODE = 1452

# Как часто сбрасывать кэши id событий и маркетов, чтобы они не пережили удаление
# строк сборщиком. Совпадает с шагом его крона (*/5).
_ID_CACHE_RESET_S = 300
...
class _StaleParent(Exception):
    """Строка потомка отвергнута внешним ключом — пакет нужно переиграть со сброшенными кэшами."""


def _db_code(exc):
    """Код ошибки MySQL из исключения pymysql (или None)."""
    if isinstance(exc, _StaleParent):
        return _FK_FAIL_CODE
...
class Listener:
    ...
    def __init__(self, bridge) -> None:
        ...
        # Первая попытка записи пакета: отказ внешнего ключа в строке не проглатывается,
        # а прерывает пакет ради повтора со сброшенными кэшами (см. _flush_batch).
        self._fk_strict  = False
        self._id_reset_at = time.monotonic()   # последний плановый сброс кэшей id
    ...
    def _apply_error(self, exc) -> None:
        """Объект не записан из-за ошибки в данных: счётчик и (первые 20) строк в журнал."""
        self._res["apply_error"] += 1
        if self._res["apply_error"] <= 20:
            print(f"\n  {C.RED}apply_error (код {_db_code(exc) or ''}): {exc}{C.RESET}")
    ...
    def _exec_rows(self, statement: str, rows: list) -> None:
        ...
        Отказ внешнего ключа на первой попытке пакета пробрасывается как
        _StaleParent: пакет переиграют со сброшенными кэшами id.
        """
        if not rows:
            return
            ...
            for i in range(0, len(rows), SQL_MULTI_ROWS):
                ...
                for row in chunk:
                    try:
                        ...
                        if self._is_transient_db(exc):
                            raise
                        if self._fk_strict and _db_code(exc) == _FK_FAIL_CODE:
                            raise _StaleParent(exc) from exc
    ...
    def _rollback(self) -> None:
        """Откатить транзакцию пакета вместе с кэшами id и дельтами, накопленными в ней."""
        try:
            self.b.db.rollback()
        except Exception:
            pass
        self.b.tx_undo()          # снять id, попавшие в кэши внутри откаченной транзакции
        self._pending_pub.clear()

    def _write_batch(self) -> None:
        """Записать пакет многострочными INSERT'ами по уровням и зафиксировать."""
        self._pending_pub.clear()
        self._write_refs()
        self._write_events()
        self._write_markets()
    ...
    def _write_slow(self) -> None:
        """Записать пакет по одному объекту, каждый под своим SAVEPOINT, и зафиксировать.

        Запасной путь, когда пакет целиком не записался из-за ошибки в данных: битый
        объект откатывается к своей точке сохранения (вместе с кэшами id и дельтами,
        сделанными при его записи) и считается в apply_error, остальные сохраняются.
        Транзиентная ошибка пробрасывается — пакет уйдёт в requeue.
        """
        b, bat = self.b, self.batch
        self._pending_pub.clear()
        levels = ((bat.cats, self._upsert_category), (bat.trns, self._upsert_tournament),
                  (bat.comps, self._upsert_competitor), (bat.events, self._upsert_event),
                  (bat.markets, self._upsert_market), (bat.outcomes, self._upsert_outcome))
        with b.db.cursor() as cur:
            for store, upsert in levels:
                for key, obj in store.values():
                    mark, npub = b.tx_mark(), len(self._pending_pub)
                    cur.execute("SAVEPOINT obj")   # одноимённая точка заменяет предыдущую
                    try:
                        upsert(key, obj)
                    except Exception as exc:
                        if self._is_transient_db(exc):
    ...
    def _flush_batch(self, channel=None) -> None:
        ...
        # Плановый сброс кэшей id — между транзакциями, пока в кэшах нет незафиксированного.
        now = time.monotonic()
        if now - self._id_reset_at >= _ID_CACHE_RESET_S:
            self._id_reset_at = now
            b.drop_id_caches()

        # Попытки по нарастающей осторожности: пакетом → пакетом со сброшенными
        # кэшами id (только после отказа внешнего ключа) → по одному объекту.
        plan = [("batch", True), ("batch", False), ("slow", False)]
        written = False
        while plan and not written:
            mode, strict = plan.pop(0)
            try:
                self._fk_strict = strict
                if mode == "batch":
                    self._write_batch()
                else:
                    self._write_slow()
                ...
                self._rollback()
                if self._is_transient_db(exc):
                    # Обрыв/failover/дедлок: сообщения НЕ теряем, возвращаем брокеру.
                    ...
                    return
                if mode == "batch" and strict:
                    if _db_code(exc) == _FK_FAIL_CODE:
                        self._res["stale_id_retry"] += 1
                        print(f"\n  {C.YELLOW}сбой БД на пакете из {msgs} сообщ.: {exc} "
                              f"→ сброс кэшей id и повтор{C.RESET}")
                        b.drop_id_caches()
                        continue
                    plan.pop(0)   # не отказ внешнего ключа: повтор тем же способом бесполезен
                if plan:
                    self._res["slow_batch"] += 1
                    print(f"\n  {C.YELLOW}сбой БД на пакете из {msgs} сообщ.: {exc} "
                          f"→ запись по одному объекту{C.RESET}")
            finally:
                self._fk_strict = False

        if not written:
            # Даже поштучно не записалось, а сбой не транзиентный. Возврат в очередь
            # повторил бы то же самое до истечения TTL — пакет подтверждаем и теряем громко.
            self._res["drop_batch"] += 1
            self._stats["err"] += msgs
            print(f"\n  {C.RED}сбой БД на пакете из {msgs} сообщ.: не записан и поштучно "
                  f"→ пакет отброшен{C.RESET}")
            bat.clear()
            self._pending_pub.clear()
            if channel is not None and tag is not None:
                try:
    ...
    def _print_resolve(self, force: bool = False) -> None:
        ...
        for k in ("resolved_event", "resolved_market", "resolved_category",
                  ...
                  "stale_id_retry", "slow_batch", "drop_batch"):
            if self._res.get(k):
