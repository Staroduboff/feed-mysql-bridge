...
COLS = ["t", "svc", "mem", "cpu", "pub", "ack", "dlv", "ready", "lag", "wage", "sage",
        ...
        # отставание от своей очереди по секундным замерам гейта: мгновенное и пик за
        # окно. Колонка lag выше — это минутный снимок ready/ack, всплески он пропускает.
        "gqlag", "gqmax"]
...
def collect_gate(now):
    ...
    """
    d = dict(gs=None, glag=None, gsil=None, ghb=None, gage=None, gdrops=None,
             gqlag=None, gqmax=None, _gate=None)
    ...
    for src, dst in (("lag", "glag"), ("sil", "gsil"), ("hb", "ghb"),
                     ("qlag", "gqlag"), ("qlag_max", "gqmax")):
        v = m.get(src)
        if v is not None:
            d[dst] = rnd(v)
    # накопленный счётчик потерянных дельт: по нему в истории видно, когда раздача
    # переставала доносить изменения до браузеров
    if m.get("pdrops") is not None:
        d["gdrops"] = int(m["pdrops"])
    return d
...
def write_catalog(cfg, now):
    ...
    вся линия prematch с названиями, имена типов маркетов/исходов. Пишется в
    WWW_DIR/catalog.json раз в минуту; это же — заготовка будущего REST-снимка.

    Горизонт линии не ограничиваем. Раньше брали 12 часов вперёд, и витрина теряла
    целые турниры: 21.09.2026 у Пижамы в дереве футбола Аргентина 65 матчей, США 37,
    Греция 25 — у нас 11, 0 и 3, при том что в базе моста лежали те же 67, 38 и 26.
    Греческий чемпионат играет раз в неделю, американский стартует через двое суток —
    в 12-часовое окно такие лиги не попадают вовсе. Цена полного горизонта мала:
    событий 1509 вместо 323, сборка каталога дольше на 0.8 с (раз в минуту),
    catalog.json 1.9 МБ против 0.4 МБ, по проводу с gzip — около 230 КБ. Нижняя
    граница в час назад остаётся: в фиде попадаются prematch-события с давно
    прошедшим стартом, они так и не перешли в live."""
    try:
        import pymysql
        import pymysql.cursors
        mc = cfg["mysql"]
...
def main():
    ...
    shell = ('<!doctype html><html lang="ru"><head><meta charset="utf-8">'
             ...
             '<link rel="icon" type="image/png" href="/icons/feed-health-32.png"><link rel="apple-touch-icon" href="/icons/feed-health-512.png"></head><body>' + frag + '</body></html>')
