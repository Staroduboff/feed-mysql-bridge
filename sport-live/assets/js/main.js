...
(function () {
  ...
  // Пиктограмма из наборов assets/icons/{dark,light} (правила — assets/css/icons.css). До 13.09.2026
  // здесь были эмодзи, их вид зависел от платформы игрока.
  const ICO = (name) => `<i class="sb-ico sb-ico--${name}" aria-hidden="true"></i>`;
  ...
  /* Тема: кнопка в шапке переключает data-theme на <html>, выбор запоминается.
     Начальное значение ставит инлайн-скрипт в <head> — до первой отрисовки,
     иначе в светлой теме мигает тёмный фон. */
  /* Тем пять: две наши (фирменная тёмная и светлая) и три перенесённые с витрины
     дизайн-концептов - «Классика», «Пульс», «Рынок». Отличаются только цветом:
     разметка и плотность у всех одна. Светлые темы дополнительно помечаются
     data-lum="light", по нему берётся светлый набор иконок. */
  const THEMES = ['dark', 'light', 'classic', 'pulse', 'market'];
  const LIGHT_THEMES = { light: 1, market: 1 };

  function applyTheme(name) {
    const root = document.documentElement;
    if (name === 'dark') root.removeAttribute('data-theme');
    else root.setAttribute('data-theme', name);
    root.setAttribute('data-lum', LIGHT_THEMES[name] ? 'light' : 'dark');
    try { localStorage.setItem('sportsbook-live-theme', name); } catch (e) { /* ignore */ }
  }
  ...
  function initTheme() {
    ...
    sel.innerHTML = THEMES.map((n) => '<option value="' + n + '"' + (n === cur ? ' selected' : '') + '>'
      + esc(T('theme_' + n)) + '</option>').join('');
  ...
  function pickFilter(alias, leagueId) {
    ...
    resetListLimits(); syncHash(); renderSidebar(); renderAllSections(true);
  }
  ...
  function renderSidebar() {
    ...
    let html = `<div class="sb-tabs">${tab('live', T('tab_live'), nLive)}${tab('line', T('tab_line'), all.length - nLive)}</div>`
      ...
      + `<span class="sb-sport__icon">${ICO('all')}</span><span class="sb-sport__name">${T('all')}</span>`
  ...
  /* Каталог отдаёт всю линию (21.09.2026 — 1442 предматча против 267 в прежнем
     12-часовом окне), и рисовать её разом нельзя: 1600 строк дают 51 тыс. узлов,
     4.7 с до готовности и 77 длинных задач в минуту — обход кнопок при каждом
     пуше идёт по всему DOM. Рисуем порциями; лимит проверяем по границе группы,
     чтобы матчи одной лиги не разъезжались на две порции с двумя заголовками. */
  const LIST_PAGE = 120;
  const listLimit = { live: LIST_PAGE, prematch: LIST_PAGE };
  const resetListLimits = () => { listLimit.live = LIST_PAGE; listLimit.prematch = LIST_PAGE; };
  let lastDrawn = 0;                       // сколько событий вошло в последний groupedRows

  function groupedRows(events, limit) {
    ...
    let html = '', drawn = 0;
    groups.forEach((list) => {
      if (limit != null && drawn >= limit) return;
  ...
  /* ── Состояние демонстрации ──────────────────────────────────────────────────
     На живом фиде сами собой случаются четыре вещи, от которых страница раньше
     просто «замирала»: обрыв связи с сервером котировок, отставший или упавший
     сборщик каталога, закрытый гейт приёма ставок и ночь без лайва. Молчать о них
     хуже всего на показе: посетитель видит цены и не знает, что они не живые.
     Полоса под шапкой показывает ОДНУ причину — самую важную из случившихся. */
  const STALE_AFTER_S = 180;                 // каталог собирается раз в минуту
  function alertState() {
    /* centrifuge переподключается сам и при обрыве висит в состоянии 'connecting',
       а не 'disconnected' (проверено подменой WS в тесте), поэтому «нет связи»
       считаем по длительности: нет соединения дольше 8 секунд — цены уже не живые */
    const cs = SB.live.connState ? SB.live.connState() : 'ok';
    const csSince = SB.live.connSince ? SB.live.connSince() : Date.now();
    if (cs !== 'ok' && Date.now() - csSince > 8000) return { kind: 'bad', text: T('alert_offline') };
    const age = SB.live.catalogAge ? SB.live.catalogAge() : null;
    if ((SB.live.catalogFailed && SB.live.catalogFailed()) || (age != null && age > STALE_AFTER_S)) {
      return { kind: 'warn', text: T('alert_stale', { min: Math.max(1, Math.round((age || 0) / 60)) }) };
    }
    const g = SB.wallet.gate;
    if (g && !g.open) return { kind: 'warn', text: T('alert_gate') };
    // сервер котировок ограничил число подписок — часть матчей живёт только на каталоге
    const ss = SB.live.subStats ? SB.live.subStats() : null;
    if (ss && ss.limit != null && ss.dropped > 0) return { kind: 'warn', text: T('alert_sublimit', { n: ss.dropped }) };
    return null;
  }
  let alertSig = '';
  function renderAlert() {
    ...
    if (!el) return;
    const st = alertState();
    const sig = st ? st.kind + '|' + st.text : '';
    if (sig === alertSig) return;            // без изменений DOM не трогаем — полоса не мигает
    alertSig = sig;
    if (!st) { el.hidden = true; el.innerHTML = ''; return; }
    el.className = 'sb-alert sb-alert--' + st.kind;
    el.innerHTML = `<span class="sb-alert__dot"></span><span class="sb-alert__text">${esc(st.text)}</span>`;
    el.hidden = false;
  }

  /* Пустая секция. В лайве это штатная ночная ситуация, а не поломка: объясняем
     словами и сразу показываем ближайшие матчи из линии — каталог держит всю
     линию, так что показывать есть что. */
  function emptyHtml(state, soon) {
    if (state !== 'live') return `<div class="sb-empty"><div class="sb-empty__title">${T('no_events')}</div></div>`;
    const head = `<div class="sb-empty">
        <div class="sb-empty__title">${T('empty_live_title')}</div>
        <div class="sb-empty__text">${soon.length ? T('empty_live_soon', { when: fmtTime(soon[0].startTs) }) : T('empty_live_text')}</div>
        <a class="sb-empty__btn" href="${navUrl('line.html')}">${T('empty_live_btn')}</a>
      </div>`;
    return head + (soon.length ? `<div class="sb-empty__sep">${T('empty_live_next')}</div>` + groupedRows(soon) : '');
  }

  /** ближайшие матчи линии — подставляются в пустой лайв.
      Берём только те, что ещё не начались: в каталоге попадаются «зависшие»
      события с давно прошедшим стартом, и показывать их как ближайшие нельзя. */
  function soonEvents(n) {
    const from = Date.now() - 5 * 60000;
    return SB.visibleEvents().filter((e) => e.state === 'prematch' && e.startTs > from).filter(inFilter)
      .sort((a, b) => a.startTs - b.startTs).slice(0, n);
  }
  ...
  function renderSection(rootSel, state, title, chipClass) {
    ...
    const rows = events.length
      ? groupedRows(events, listLimit[state])
      : emptyHtml(state, state === 'live' ? soonEvents(8) : []);
    const drawn = events.length ? lastDrawn : 0;
    root.innerHTML = `
      <div class="sb-section sb-section--${state === 'live' ? 'live' : 'prematch'}">
        <div class="sb-section__head">
          <span class="sb-section__title">${ICO(state === 'live' ? 'live' : 'line')}${title}</span>
        ...
        ${drawn < events.length
          ? `<button class="sb-more" data-more="${state}">${T('show_more', { n: events.length - drawn })}</button>`
          : ''}
      </div>`;
    attachRowClicks(root);
    wireMore(root, state, events.length);
  }

  /* Хвост списка: сам дорисовывает следующую порцию, когда попадает в поле зрения.
     Кнопка остаётся рабочей и без IntersectionObserver — на старых браузерах
     список просто листается вручную, а не обрывается. */
  const moreObs = {};
  function wireMore(root, state, total) {
    if (moreObs[state]) { moreObs[state].disconnect(); moreObs[state] = null; }
    const el = $(`[data-more="${state}"]`, root);
    if (!el) return;
    const grow = () => {
      listLimit[state] = Math.min(total, (listLimit[state] || LIST_PAGE) + LIST_PAGE);
      renderAllSections(true);
    };
    el.addEventListener('click', grow);
    if (typeof IntersectionObserver === 'function') {
      moreObs[state] = new IntersectionObserver((ents) => {
        if (ents.some((x) => x.isIntersecting)) { moreObs[state].disconnect(); moreObs[state] = null; grow(); }
      }, { rootMargin: '200px' });
      moreObs[state].observe(el);
    }
  }
  ...
  function renderHot(rootSel) {
    ...
    root.innerHTML = `
      <div class="sb-section sb-section--hot">
        <div class="sb-section__head">
          <span class="sb-section__title">${ICO('hot')}${T('hot')}</span>
    ...
    if (!bets.length) return sub + `
      <div class="sb-slip__list">
        <div class="sb-slip__empty">
          <div class="sb-slip__empty-icon">${ICO('bets')}</div>
  ...
  function renderSlip(force) {
    ...
    const couponHtml = `
      ...
      <div class="sb-slip__list">
        ${items.length === 0 ? `
          <div class="sb-slip__empty">
            <div class="sb-slip__empty-icon">${ICO('coupon')}</div>
      ...
      ${items.length > 0 ? `
        <div class="sb-slip__foot">
          <div class="sb-slip__stake-presets">
            ${SB.stakePresets().map((n) => `<button data-stake-set="${n}"${stake === n ? ' class="active"' : ''}>${n}</button>`).join('')}
          </div>
          ...
          ${anyUnavailable ? `<div class="sb-slip__warn" style="font-size:11px;color:var(--orange);text-align:center">${ICO('warn')} ${T('unavailable_warn')}</div>` : ''}
  ...
  function renderAllOddButtons() {
    ...
    $$('.sb-odd').forEach((btn) => {
      ...
      if (!idx) {
        // Исхода больше нет в индексе (каталог сменил главный маркет строки — например, линия
        // форы переехала): до перестройки строк такая кнопка не должна оставаться кликабельной
        // со старым кэфом. Замер 13.09.2026: 192 эпизода «WS закрыл, экран открыт» до 72 с,
        // почти все — фора, старт сразу после ежеминутного каталога.
        btn.classList.add('sb-odd--disabled'); btn.classList.remove('sb-odd--active'); btn.disabled = true;
        return;
      }
  ...
  function renderAllSections(force) {
    ...
    const shown = modeEvents().filter(inFilter);
    // при пустом лайве в секции показываются ближайшие матчи линии — геометрию колонок
    // считаем по ним, иначе строки поехали бы по пустому набору маркетов
    layouts = buildLayouts(shown.length ? shown : soonEvents(8));
    renderHot('#sb-hot');
  ...
  window.SBInit = function (mode) {
    ...
    SB_I18N.apply(); SB_I18N.switcher($('#sb-lang')); keepNavParams(); initTheme();
    ...
    SB.live.initIndex((kind, info) => {
      ...
      renderAlert();
    }, status).then(() => { renderSidebar(); renderAllSections(true); renderSlip(true); renderBalance(); renderAlert(); });
    setInterval(renderAlert, 5000);         // возраст каталога и длительность обрыва растут сами
  ...
  window.SBInitEvent = function (eventId) {
    ...
    // Быстрый фильтр маркетов — как на странице события Пижамы (sport-shell,
    // src/components/EventBrowser.tsx, MARKET_QUICK_FILTERS): набор вкладок и
    // шаблоны взяты оттуда один в один, чтобы игрок, ходящий между витринами,
    // видел одно и то же поведение. Вкладка показывается, только если у события
    // есть подходящие маркеты, поэтому на теннисе их не будет вовсе.
    //
    // Сопоставление идёт по строке из кода маркета, кода периода и их
    // локализованных названий, из которой выброшено всё, кроме букв и цифр, —
    // так «Удары в створ» и «Shots on Target» ловятся одним набором шаблонов
    // независимо от языка витрины.
    const mqNorm = (v) => String(v || '').toLowerCase().normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '').replace(/ё/g, 'е').replace(/[^a-zа-я0-9]+/g, '');
    const MQ_ON_TARGET = ['shotontarget', 'shotsontarget', 'targetshot', 'targetshots', 'ударвствор', 'ударывствор',
      'tircadre', 'tirscadres', 'tiroapuerta', 'tirosapuerta', 'chutenoalvo', 'chutesnoalvo', 'choutsoukad'];
    const MQUICK = [
      { k: 'half1', t: 'mf_half1', periods: ['half1', '1sthalf', 'firsthalf', '1half', '1time', '1йтайм', 'первыйтайм'] },
      { k: 'half2', t: 'mf_half2', periods: ['half2', '2ndhalf', 'secondhalf', '2half', '2time', '2йтайм', 'второйтайм'] },
      { k: 'corners', t: 'mf_corners', patterns: ['corner', 'corners', 'углов', 'угловые', 'escanteio', 'escanteios', 'kone'] },
      { k: 'cards', t: 'mf_cards', patterns: ['yellowcard', 'yellowcards', 'booking', 'bookings', 'желт', 'cartonjaune', 'cartonjaunes', 'tarjetaamarilla', 'tarjetasamarillas', 'cartaoamarelo', 'cartoesamarelos', 'katonjon'] },
      { k: 'shots_target', t: 'mf_shots_target', patterns: MQ_ON_TARGET },
      { k: 'fouls', t: 'mf_fouls', patterns: ['foul', 'fouls', 'фол', 'фолы', 'faute', 'fautes', 'falta', 'faltas', 'fot'] },
      { k: 'offsides', t: 'mf_offsides', patterns: ['offside', 'offsides', 'офсайд', 'офсайды', 'horsjeu', 'fueradejuego', 'fuerasdejuego', 'impedimento', 'impedimentos', 'ofsayd'] },
      { k: 'shots', t: 'mf_shots', patterns: ['shot', 'shots', 'удар', 'удары', 'tir', 'tirs', 'tiro', 'tiros', 'chute', 'chutes', 'chout'], exclude: MQ_ON_TARGET },
      { k: 'saves', t: 'mf_saves', patterns: ['save', 'saves', 'сейв', 'сейвы', 'arret', 'arrets', 'atajadas', 'atajada', 'defesa', 'defesas', 'sovtaj'] },
    ];
    const mqHay = (m) => [m._p, m._n, m.name, SB.live.marketGroupTitle(m), SB.live.periodTitle(m)]
      .filter(Boolean).map(mqNorm).join(' ');
    function mqMatch(m, f) {
      const h = mqHay(m);
      if (f.exclude && f.exclude.some((p) => h.includes(p))) return false;
      if (f.periods && f.periods.some((p) => h.includes(p))) return true;
      return !!(f.patterns && f.patterns.some((p) => h.includes(p)));
    }
    const tabPred = (m) => {
      if (mfilter.tab === 'all') return true;
      const f = MQUICK.find((x) => x.k === mfilter.tab);
      return f ? mqMatch(m, f) : true;
    };
    ...
    const marketsFiltered = (all) => all.filter((m) => periodPred(m) && tabPred(m) && qPred(m));
    let barSig = '';
    function renderMarketBar(all) {
      ...
      if (!bar.dataset.ready) {
        ...
        // Делегирование, потому что кнопки пересобираются при смене набора вкладок.
        bar.querySelector('.sb-mbar__tabs').addEventListener('click', (e) => {
          const b = e.target.closest('.sb-mbar__tab'); if (!b) return;
          mfilter.tab = b.dataset.tab; renderMarkets();
        });
        let qt = null;
      ...
      // Вкладка живёт, только если у события есть её маркеты: на теннисе останется
      // одна «Все», на футболе — набор из скрина Пижамы.
      const tabs = [['all', T('tab_all')]].concat(
        MQUICK.filter((f) => all.some((m) => mqMatch(m, f))).map((f) => [f.k, T(f.t)])
      );
      const tabsEl = bar.querySelector('.sb-mbar__tabs');
      const tsig = tabs.map((x) => x[0]).join('|');
      if (tabsEl.dataset.sig !== tsig) {
        tabsEl.dataset.sig = tsig;
        tabsEl.innerHTML = tabs.map(([k, l]) => `<button class="sb-mbar__tab" data-tab="${k}">${esc(l)}<b></b></button>`).join('');
        if (!tabs.some((x) => x[0] === mfilter.tab)) mfilter.tab = 'all';
        barSig = '';
      }
      const inPeriod = all.filter((m) => periodPred(m) && qPred(m));
      const counts = tabs.map(([k]) => (k === 'all' ? inPeriod.length
        : inPeriod.filter((m) => mqMatch(m, MQUICK.find((f) => f.k === k))).length)).join(',');
    ...
    function renderMarkets(force) {
      ...
      if (force || !markets.length || !root.querySelector('.sb-mgroup')) {
        root.innerHTML = markets.length ? items.map(itemHtml).join('')
          : `<div style="padding:24px;text-align:center;color:var(--foreground)">${
              all.length ? T('no_markets_filter')
              : SB.marketsTotal(event) ? T('no_markets_open')   // все закрыты — опасный момент в матче
              : T('markets_loading')}</div>`;
    ...
    SB.live.initEvent(eventId, (kind, info) => {
      ...
      renderAlert(); setInterval(renderAlert, 5000);
