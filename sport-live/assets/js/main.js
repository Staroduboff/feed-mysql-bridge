...
(function () {
  ...
  function initTheme() {
    ...
    btn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('sportsbook-live-theme', next); } catch (e) { /* ignore */ }
    });
  }
  ...
  /* Пустая секция. В лайве это штатная ночная ситуация, а не поломка: объясняем
     словами и сразу показываем ближайшие матчи из линии — каталог держит предматч
     на 12 часов вперёд, так что показывать есть что. */
    ...
    root.innerHTML = `
      <div class="sb-section sb-section--${state === 'live' ? 'live' : 'prematch'}">
        ...
        <div class="sb-table sb-table--x">
          ${events.length ? groupedRows(events) : emptyHtml(state, state === 'live' ? soonEvents(8) : [])}
        </div>
      </div>`;
