...
(function () {
  ...
  // Пиктограммы видов спорта — PNG-наборы под тему (assets/css/icons.css, tools/icons/gen_icons.py);
  // виду спорта без своей картинки достаётся общая «медаль». До 13.09.2026 здесь были эмодзи.
  const SPORT_ICONS = new Set([1, 2, 3, 4, 5, 6, 7, 8, 12, 13, 14, 15]);
  const sportIcon = (id) => '<i class="sb-ico sb-ico--' + (SPORT_ICONS.has(+id) ? 's' + id : 'sport') + '" aria-hidden="true"></i>';
  ...
  const subs = {};
  let cf = null;
  // Отказы подписки. Centrifugo ограничивает число каналов на клиента (client.channel_limit;
  // 14.09.2026 на *10 по умолчанию было 128 при наших 8 + до 400 каналах — подписки сверх
  // лимита молча отвергались кодом 106, и события без подписки обновлялись только каталогом).
  // Лимит запоминаем и выше него не пробуем; сколько живых матчей осталось без подписки —
  // показывает полоса состояния.
  let subErrors = 0;            // отказов подписки всего
  let subLimit = null;          // сколько каналов сервер разрешил (после первого 106)
  let subDropped = 0;           // живых матчей, не попавших в подписку из-за лимита
  // Подписка по видимой области. Канал event:{id} несёт ВСЕ маркеты и исходы
  // события, а в строке списка видно три маркета. Подписка на все живые события
  // дала на вечернем пике 17.09.2026 2.6 млн объектов обновлений за три часа:
  // модель успевала, отрисовка нет — 105 эпизодов расхождения экрана с линией
  // дольше 10 с, максимум 382 с, 84 из них в получасовом окне пика; фид в это
  // время был чист. Поэтому берём только то, что игрок видит, плюс запас за
  // краями окна — так же устроен sport-shell Пижамы, через который в том же
  // прогоне прошло 12 тыс. объектов против наших 2.6 млн.
  const VIEW_OVERSCAN_PX = 400;   // запас сверху и снизу от окна
  const VIEW_DEBOUNCE_MS = 300;
  const HISTORY_LIMIT = 300;      // = history_size канала event в Centrifugo
  const viewEv = new Set();       // id событий, видимых сейчас (с запасом)
  let viewTimer = null, viewOn = false;
  let histReplayed = 0, histErrors = 0;
  let onChange = () => {};      // вызывается адаптером при изменениях: (kind) kind = 'odds'|'sections'|'event'|'conn'

  /* Состояние демонстрации — его показывает полоса под шапкой (renderAlert в main.js):
     связь с сервером котировок, свежесть каталога и то, удалась ли последняя загрузка.
     Всё это на живом фиде случается само собой (ночь по Гаити, обрыв WS, упавший
     сборщик), и молча замершие цены на показе выглядят хуже честной плашки. */
  let connState = 'connecting';   // 'connecting' | 'ok' | 'bad'
  let connSince = Date.now();     // когда состояние связи стало текущим
  let catalogT = 0;               // время сборки каталога (мс, часы сервера)
  let catalogFailed = false;      // последняя попытка обновить каталог не удалась
  ...
  function ensureSport(s) {
    ...
    if (!sp) { sp = { id: +s.id, alias: sportAlias(s.id), label: DICT.sport[s.n] || (LN === 'ru' && RU_SPORT[s.id]) || s.n, icon: sportIcon(s.id), order: 999999, excluded: false }; SPORTS.push(sp); }
  ...
  function cmpRank(a, b) {
    ...
    if (a.state === 'live') {
      ...
      //
      // При РАВНОМ времени начала у них решает порядок обхода дерева в памяти WS:
      // категории и турниры — в порядке добавления (ключи-хэши, обход объекта), события внутри
      // турнира — по возрастанию id (числовые ключи объекта обходятся по возрастанию).
      // Порядок добавления турниров воспроизвести нельзя — это история процесса, не данные фида;
      // берём вместо него веса стран и турниров из админки, они детерминированы и не «прыгают».
      // А вот порядок событий внутри турнира воспроизводится точно — по id события.
      const pa = a._lp == null ? 1e9 : a._lp, pb = b._lp == null ? 1e9 : b._lp;
      return (pa - pb) || (a.startTs - b.startTs)
        || (a._co - b._co) || (a._to - b._to)
        || (la.country || '').localeCompare(lb.country || '')
        || (la.name || '').localeCompare(lb.name || '')
        || (a.id - b.id);
    }
  ...
  /** можно ли поставить хоть на что-то в этом маркете */
  const marketPlayable = (m) => !!m._open && !m._removed && m.selections.some((s) => s.available);

  /* Сколько держим на экране маркет, который закрылся у игрока на глазах.
     Мгновенное скрытие дёргало бы страницу: замер 21.09.2026 на 40 матчах лайва —
     618 закрытий за 6 минут (103 в минуту), медиана закрытия 17.4 с, 395 возвратов
     из 554 короче 30 секунд. Блок исчезает и через полминуты возвращается, всё
     нижнее прыгает, а игрок в этот момент целится в кнопку. */
  const HIDE_CLOSED_MS = 20000;

  /** показывать ли маркет на странице события */
  function marketVisible(event, m) {
    if (marketPlayable(m)) return true;
    // маркет, которого игрок ни разу не видел доступным, не показываем ни секунды
    const closedAt = event._closedAt && event._closedAt[m._h];
    return closedAt != null && Date.now() - closedAt < HIDE_CLOSED_MS;
  }

  /** отметить, что стало с доступностью маркетов после пересборки кэша */
  function trackPlayability(event) {
    const now = Date.now();
    const seen = event._seenOpen || (event._seenOpen = {});
    const closed = event._closedAt || (event._closedAt = {});
    (event.allMarketsCache || []).forEach((m) => {
      if (marketPlayable(m)) { seen[m._h] = true; delete closed[m._h]; return; }
      if (seen[m._h] && closed[m._h] == null) {
        closed[m._h] = now;
        // за время выдержки новых дельт может не прийти — будим перерисовку сами,
        // иначе блок останется висеть серым до следующего изменения кэфов
        if (!event._hideTimer) {
          event._hideTimer = setTimeout(() => { event._hideTimer = null; schedule('event'); }, HIDE_CLOSED_MS + 300);
        }
      }
    });
  }
  ...
  function toMarket(event, mk) {
    ...
    sortOc(mk.oc || []).forEach((o) => {
      if (o.removed) return;
      o._mv = mk.v || '';
      const s = { id: selId(event.id, mk.h, o.h), label: selLabel(mk.n, o), odds: o.price != null ? +o.price : 0, available: !!mk.open && !mk.removed && o.status === 1 && o.price != null, _h: o.h, _status: o.status, _result: o.result || 0, _t: o.t, _n: o.n || '', _v: o.v || '' };
      m.selections.push(s);
  ...
  function upsertEvent(e) {
    ...
    if (e.main) {
      ...
      const old = new Map(), oldMk = new Map();
      event.mainMarkets.forEach((m) => { oldMk.set(m._h, m); m.selections.forEach((s) => { old.set(s.id, s); selIndex.delete(s.id); }); });
      event.mainMarkets = mainMarketsFrom(event, e.main);
      event.mainMarkets.forEach((m) => {
        // Состояние самого маркета (open/removed) тоже могло прийти по WS позже сборки
        // каталога — иначе следующая дельта цены пересчитала бы доступность исходов по
        // устаревшему open и ячейка гасла бы до следующей дельты маркета (замер 13.09.2026:
        // эпизоды «WS открыл, экран закрыт» до двух минут).
        const om = oldMk.get(m._h);
        if (om && om._wsAt && om._wsAt > catT) { m._open = om._open; m._removed = om._removed; m._wsAt = om._wsAt; }
        else { const c = event._wsMk && event._wsMk[m._h]; if (c && c.t > catT) { m._open = c.open; m._removed = c.removed; m._wsAt = c.t; } }
        m.selections.forEach((s) => {
          const o = old.get(s.id);
          if (o && o._wsAt && o._wsAt > catT) { s.odds = o.odds; s._status = o._status; s._result = o._result; s._wsAt = o._wsAt; }
          else { const c = event._wsOc && event._wsOc[m._h + '|' + s._h]; if (c && c.t > catT) { s._status = c.status; if (c.price != null) s.odds = c.price; if (c.removed) s._status = 0; s._wsAt = c.t; } }
          // доступность — из самых свежих известных составляющих, по тому же правилу, что в дельтах
          s.available = !!m._open && !m._removed && s._status === 1 && s.odds > 0;
        });
      });
    }
    event._snap = !!e.snap;
  ...
  // Партнёрская витрина показывает весь фид, включая страны и турниры, не открытые
  // спортаналитиком в админке Спорта (cfg:c.excl / cfg:t.excl — на 16.09.2026 это 43 %
  // событий каталога). Пижама их прячет: она торгует по своей линии и сама отбирает,
  // что принимать. Партнёру же продаётся полнота фида, и что из неё выставлять, решает
  // он сам, поэтому здесь исключения админки не применяются. Порядок сортировки из
  // админки сохраняется: неоткрытые лиги уходят в хвост своего вида спорта (order 999999).
  // Вернуть набор Пижамы — поставить false.
  const SHOW_ADMIN_EXCLUDED = true;
  const hiddenByAdmin = (e) => !SHOW_ADMIN_EXCLUDED && e._x;
  ...
  function pickHot() {
    ...
    const ok = (e) => !e._finished && !e._removed && !hiddenByAdmin(e) && e.mainMarkets.length > 0;
  ...
  // Завершённые матчи и матчи без главных маркетов не показываются — как на live-странице
  // Пижамы. Исключения админки здесь не применяются, см. SHOW_ADMIN_EXCLUDED выше. Событие,
  // пришедшее только по WS (без каталога), появится в списке, когда каталог (раз в минуту)
  // отдаст его главные маркеты.
  const isVis = (e) => !e._removed && !e._finished && !hiddenByAdmin(e) && e.mainMarkets.length > 0;
  ...
  async function loadCatalog() {
    ...
    catalogT = catT; catalogFailed = false;
  ...
  function rebuildAll(event) {
    ...
    // порядок маркетов — как в админке (cfg:m по виду спорта): позиция стабильна, и
    // маркет, вернувшийся из закрытого состояния, встаёт на своё место, а не в конец.
    // В кэше держим и закрытые: они нужны купону и колонкам таблицы (там ячейка
    // остаётся серой), а страница события берёт их через buildAllMarkets с фильтром
  ...
  function onDelta(d) {
    ...
    if (d.ch === 'markets') {
      ...
      // Состояние запоминается для ЛЮБОГО маркета события, не только главного: каталог
      // может через минуту поставить этот маркет в слот строки (переехала линия форы), и
      // без этой памяти он показался бы в состоянии на момент сборки каталога, а не в
      // текущем. Замер 13.09.2026: все 97 опасных эпизодов после правок А–В — ровно это.
      (event._wsMk = event._wsMk || {})[h] = { open: mk.open, removed: mk.removed, t: srvNow() };
      // главные маркеты строки
      const main = event.mainMarkets.find((m) => m._h === h);
      if (main) { main._open = mk.open; main._removed = mk.removed; main._wsAt = srvNow(); main.selections.forEach((s) => { s.available = mk.open && !mk.removed && s._status === 1 && s.odds > 0; s._wsAt = srvNow(); }); schedule('odds'); }
    ...
    if (d.ch === 'outcomes') {
      ...
      (event._wsOc = event._wsOc || {})[h + '|' + oh] = { status: o.status, removed: !!o.removed, price, t: srvNow() };
      const main = event.mainMarkets.find((m) => m._h === h);
      if (main) {
        const s = main.selections.find((x) => x._h === oh);
        if (s) { s.odds = price != null ? price : s.odds; s._status = o.status; s._result = o.result || 0; s.available = main._open && !main._removed && o.status === 1 && !o.removed && price != null; s._wsAt = srvNow(); schedule('odds'); }
      }
      if (event._markets) {
  ...
  function subscribe(ch, catchUp) {
    if (!cf || subs[ch]) return;
    const s = cf.newSubscription(ch);
    // Пока тянем историю, живые дельты копим, а не применяем: иначе история,
    // пришедшая следом, затрёт их своим более старым состоянием.
    s._queue = catchUp ? [] : null;
    s.on('publication', (ctx) => {
      const d = ctx.data || {};
      if (s._queue) { if (s._queue.length < 2000) s._queue.push(d); return; }
      onDelta(d);
    });
    if (catchUp) s.on('subscribed', () => { if (s._queue) replayHistory(ch, s); });
    s.on('error', (ctx) => {
      const err = (ctx && ctx.error) || {};
      subErrors++;
      if (subErrors <= 3) console.warn('sport-live: подписка', ch, 'отклонена:', err.code, err.message);
      if (err.code === 106) {
        // лимит каналов на клиента: снимаем эту подписку, иначе клиент будет пробовать её
        // бесконечно, и дальше этого числа не подписываемся до переподключения
        subLimit = Math.max(1, Object.keys(subs).length - 1);
        unsubscribe(ch);
        onChange('conn', { state: connState });
      }
    });
  ...
  function subscribeEvent(id) {
    // Событие, ушедшее в лайв: подписываемся, только если строка на экране.
    // Остальные догонят историю канала, когда игрок до них доскроллит.
    if (!viewEv.has(+id)) return;
    if (Object.keys(subs).filter((c) => c.startsWith('event:')).length >= MAX_EVENT_SUBS) return;
    if (subLimit != null && Object.keys(subs).length >= subLimit) { subDropped++; return; }
    subscribe('event:' + id, true);
  }
  // Пересчёт видимых строк. Считаем геометрией, а не IntersectionObserver:
  // список перерисовывается целиком, и следить за жизнью узлов дороже, чем раз
  // в 300 мс пройти по строкам.
  function recomputeVisible() {
    viewTimer = null;
    if (!viewOn) return;
    const h = window.innerHeight || 0, next = new Set();
    document.querySelectorAll('[data-eid]').forEach((n) => {
      const id = +(n.getAttribute('data-eid') || 0);
      if (!id) return;
      const b = n.getBoundingClientRect();
      if (b.bottom >= -VIEW_OVERSCAN_PX && b.top <= h + VIEW_OVERSCAN_PX) next.add(id);
    });
    let same = next.size === viewEv.size;
    if (same) { for (const id of next) { if (!viewEv.has(id)) { same = false; break; } } }
    if (same) return;
    viewEv.clear(); next.forEach((id) => viewEv.add(id));
    syncSubs('index');
  }

  function scheduleViewSync() {
    // Таймер намеренно не перезапускаем: при непрерывной прокрутке или плотном
    // потоке пушей пересчёт иначе откладывался бы бесконечно (грабли sport-shell,
    // их правили отдельным коммитом 15.09.2026).
    if (viewTimer !== null) return;
    viewTimer = setTimeout(recomputeVisible, VIEW_DEBOUNCE_MS);
  }

  function startViewTracking() {
    if (viewOn) return;
    viewOn = true;
    addEventListener('scroll', scheduleViewSync, { capture: true, passive: true });
    addEventListener('resize', scheduleViewSync);
    if (typeof MutationObserver === 'function') {
      new MutationObserver(scheduleViewSync).observe(document.body, { childList: true, subtree: true });
    }
    recomputeVisible();
  }

  // Строка вошла в окно: доигрываем то, что канал опубликовал, пока мы не были
  // подписаны (history_ttl 60 с перекрывает минутный цикл каталога), и только
  // потом применяем живые дельты, накопленные за время догона. Порядок важен:
  // у исходов в фиде нет версии, и старая история затёрла бы свежий пуш.
  async function replayHistory(ch, s) {
    let pubs = [];
    try {
      const r = await s.history({ limit: HISTORY_LIMIT, reverse: false });
      pubs = (r && r.publications) || [];
    } catch (e) {
      histErrors++;
      if (histErrors <= 3) console.warn('sport-live: история канала', ch, 'недоступна:', e && e.message);
    }
    pubs.forEach((p) => onDelta(p.data || {}));
    histReplayed += pubs.length;
    const q = s._queue || [];
    s._queue = null;
    q.forEach((d) => onDelta(d));
    if (pubs.length || q.length) schedule('odds');
  }

  function syncSubs(mode, eventId) {
    const want = new Set();
    if (mode === 'index') {
      // sport:{id} несёт только дельты событий — счёт, статус, появление новых.
      // Это дёшево и нужно всему списку, поэтому берём все виды спорта.
      SPORTS.forEach((s) => want.add('sport:' + s.id));
      // event:{id} несёт маркеты и исходы — только для строк на экране.
      let n = 0;
      viewEv.forEach((id) => {
        if (n >= MAX_EVENT_SUBS) return;
        const e = byId.get(id);
        if (e && !e._finished && !e._removed) { want.add('event:' + id); n++; }
      });
    ...
    subDropped = 0;
    want.forEach((ch) => {
      if (subLimit != null && !subs[ch] && Object.keys(subs).length >= subLimit) { if (ch.startsWith('event:')) subDropped++; return; }
      subscribe(ch, ch.startsWith('event:'));
    });
  }
  ...
  function connect(statusEl) {
    ...
    const mark = (st) => { if (connState !== st) { connState = st; connSince = Date.now(); onChange('conn', { state: st }); } };
    cf.on('connecting', () => { set('', T('ws_connecting')); mark('connecting'); });
    cf.on('connected', () => { set('ok', T('ws_ok')); mark('ok'); subLimit = null; subDropped = 0; });
    cf.on('disconnected', () => { set('bad', T('ws_bad')); mark('bad'); });
    cf.connect();
  }
  ...
  // Быстрые суммы ставки. Лежат здесь, а не в разметке купона, потому что первая из них —
  // ещё и сумма по умолчанию для нового купона: так значение и подсветка кнопки не разъезжаются.
  ...
  window.SB = {
    ...
    sport: (alias) => SPORTS.find((s) => s.alias === alias) || { id: 0, alias, label: alias, icon: sportIcon(0) },
    league: (id) => LEAGUES.find((l) => l.id === id) || { id, sport: '', country: '', name: '' },
    eventById: (id) => byId.get(+id) || null,
    visibleEvents,
    /* Маркет попадает на страницу события, только если по нему можно поставить:
       он открыт, не снят и в нём есть хотя бы один исход с ценой. Закрытые блоки
       раньше висели серыми — в лайве это 73 % всех маркетов (замер 21.09.2026:
       3232 из 4427), на отдельных матчах 286 из 290, и игрок листал страницу
       впустую. Так же поступал спорт старой Пижамы: маркет с op=false в дерево
       не попадал (sport/Betting, V-MarketsBrowser.groupedMarkets).

       Фильтруем на отдаче, а не в allMarketsCache: индекс исходов selIndex должен
       остаться полным, иначе купон потеряет уже выбранный исход, когда его маркет
       закроется, — а он обязан показать игроку, что ставка больше не идёт. */
    buildAllMarkets: (event) => (event.allMarketsCache || []).filter((m) => marketVisible(event, m)),
    /** сколько маркетов пришло всего, вместе с закрытыми: отличить «все закрылись»
        от «ещё не загрузились» */
    marketsTotal: (event) => ((event && event.allMarketsCache) || []).length,
    loadSlip,
    toggleInSlip(sid) {
      const s = loadSlip();
      const i = s.selections.indexOf(String(sid));
      if (i >= 0) s.selections.splice(i, 1);
      else {
        // Первый исход в пустом купоне сразу получает сумму по умолчанию: игрок видит
        // заполненное поле и возможный выигрыш, не набирая сумму руками. Уже введённую
        // сумму не трогаем — ни при добавлении следующих исходов, ни если игрок набрал свою.
        if (!s.selections.length && !s.stake) s.stake = STAKE_PRESETS[0];
        s.selections.push(String(sid));
      }
      saveSlip(s);
      return s;
    },
    ...
    /** быстрые суммы ставки; первая — сумма по умолчанию для нового купона */
    stakePresets: () => STAKE_PRESETS.slice(),
    ...
    live: {
      /** состояние связи с сервером котировок: 'connecting' | 'ok' | 'bad' */
      connState: () => connState,
      /** когда связь перешла в текущее состояние (мс) */
      connSince: () => connSince,
      /** возраст каталога в секундах по часам СЕРВЕРА (SRV_SKEW — поправка на
       *  расхождение часов: телефон с убежавшим временем иначе показывал бы
       *  ложное «линия отстала»); null — каталог ещё не загружен */
      catalogAge: () => (catalogT ? Math.max(0, ((Date.now() + SRV_SKEW) - catalogT) / 1000) : null),
      /** последняя попытка обновить каталог не удалась */
      catalogFailed: () => catalogFailed,
      /** подписки WS: сколько каналов, сколько отказов, лимит сервера (если упёрлись) и
       *  сколько живых матчей из-за него без живых обновлений */
      subStats: () => ({ channels: Object.keys(subs).length,
      events: Object.keys(subs).filter((c) => c.startsWith('event:')).length,
      visible: viewEv.size, replayed: histReplayed, histErrors,
      errors: subErrors, limit: subLimit, dropped: subDropped }),
      /** главная: каталог + WS; cb(kind) — просьба перерисовать */
      async initIndex(cb, statusEl) {
        onChange = cb;
        await loadDict();
        await loadCatalog();
        connect(statusEl); syncSubs('index'); startViewTracking();
        walletRefresh(true); setInterval(() => walletRefresh(false), 30000);
        setInterval(async () => {
          try { await loadCatalog(); syncSubs('index'); onChange('sections'); }
          catch (e) { catalogFailed = true; onChange('conn', { state: connState }); }   // сборщик или раздача недоступны
        }, 60000);
