...
палитре сайта, отдельно под тёмную и светлую тему:
  ...
  dark  — золотой знак (#FFB62F) с толстой navy-обводкой (#0A1A2F), внутренние детали
          тем же navy; ложится на тёмный фон страницы (вариант D5);
  light — сплошной navy (#10263A) одним цветом, детали — просветы фона; ложится на
          светлый фон (вариант L1).
Оба набора без бирюзы. Светлый рисуется СВОИМ промптом, не перекрашиванием тёмного:
image-to-image оставлял на знаке тёмные фрагменты исходника («грязь»).
Стили выбраны владельцем 16.09.2026 из десяти вариантов на одном сюжете.
...
product-showcase/assets/generated/_usage.jsonl.
  ...
  python gen_icons.py light [name …]   # светлый набор → raw/light/<name>.png
...
ICONS = [
    ...
    ('s4',  'a tennis racket seen from the front with a tennis ball beside its head; the racket and the ball are the only shapes, nothing behind them'),
    ...
    ('moon',   'a plain crescent moon, one smooth solid crescent shape and nothing else, no craters, no stars, no frame or border around it'),
...
COMMON = (
    ...
    "canvas, nothing else on it: no rounded square tile, plate, badge, card or frame behind or "
    "around the symbol, the magenta must touch all four edges of the canvas. "
    "Must stay readable when scaled down to 16 pixels. "
    "Absolutely no text, no letters, no numbers, no words, no watermark."
)
# Вариант D5: золотая заливка, толстая navy-обводка по контуру, внутренние детали тем же navy.
DARK = (
    "Icon subject: %s.\n\nColors: main fill warm gold (#FFB62F), a thick deep navy (#0A1A2F) "
    "outline around the symbol and the same navy for all inner detail lines. Exactly two colors, "
    "no other colors at all, no teal, no cyan, no turquoise, no green, no blue except that navy. " + COMMON
)
# Вариант L1: один сплошной navy, детали — просветы фона (после кеинга станут прозрачными).
LIGHT = (
    "Icon subject: %s.\n\nColors: the whole symbol is one single flat color, deep navy (#10263A). "
    "Inner detail lines are not drawn in another color: they are gaps that show the magenta "
    "background through the symbol. Exactly one color on the symbol, no second fill, no outline "
    "around the symbol, no teal, no cyan, no turquoise, no gold, no white. " + COMMON
)
...
def task(key, theme, name, subject):
    ...
    # Оба набора рисуются с нуля своим промптом: перекрашивание тёмного варианта
    # (image-to-image) оставляло на светлом знаке тёмные фрагменты исходника.
    prompt, ref = (DARK if theme == 'dark' else LIGHT) % subject, None
