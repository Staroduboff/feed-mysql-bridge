...
function noteChange(s, key, before, t, src) {
  ...
  s.pending.set(key, { t, to: after, src });
  s.truthT.set(key, t);
}
...
// ── экраны ────────────────────────────────────────────────────────────────
const OUR_DOM = () => {
  const cells = {}; const model = {};
  document.querySelectorAll('.sb-odd[data-sid]').forEach((b) => { if (b.offsetParent === null) return; const sid = b.dataset.sid; if (cells[sid] === undefined) cells[sid] = !b.disabled; else if (cells[sid] !== !b.disabled) cells[sid] = null; });
  try { window.SB.indexSelections().forEach((v, sid) => { if (cells[sid] !== undefined) model[sid] = !!v.selection.available; }); } catch (e) { /* ignore */ }
  return { cells, model, conn: (document.querySelector('.sb-conn') || {}).className || '' };
};
const PZ_DOM = (names) => {
  ...
  rows.forEach((r) => {
    ...
    mkts.forEach((m) => {
      ...
      cells.forEach((cell) => {
        ...
        const key = String(e.id) + '|' + m.k + '|' + cell.oc.oc; out.set(key, !cell.c.d); (s.lastText = s.lastText || new Map()).set(key, cell.c.t);
      });
    });
  });
  return out;
...
function compare(s, dom, now) {
  ...
  dom.forEach((open, key) => {
    ...
    if (open !== tr) {
      ...
      const p = s.pending.get(key); if (p && p.to === open) { (p.src === 'ws' ? s.delays : (s.delaysSnap = s.delaysSnap || [])).push(now - p.t); s.pending.delete(key); }
    }
  });
  ...
  s.pending.forEach((p, key) => { if (now - p.t > 120000) s.pending.delete(key); });
}
function cross(now) {
...
function siteReport(s) {
  ...
  lines.push(`Экран: опросов ${s.samples}, ячеек в среднем ${s.samples ? Math.round(s.cells / s.samples) : 0}` + (s.name === 'Пижама' ? `, привязано групп ${s.mapped}, не привязано групп ${s.unmapped}, ячеек без исхода ${s.unmappedCells || 0}` : `, модель≠экран: ${s.modelMis}`));
  ...
  lines.push(`Задержка отрисовки смены состояния из потока (пуш/дельта → экран): n=${s.delays.length}, медиана ${fmt(q(s.delays, 0.5))}, p95 ${fmt(q(s.delays, 0.95))}, максимум ${fmt(q(s.delays, 1))}`);
  const ds = s.delaysSnap || [];
  lines.push(`Задержка отрисовки смены из снимка (live2/каталог → экран): n=${ds.length}, медиана ${fmt(q(ds, 0.5))}, p95 ${fmt(q(ds, 0.95))}, максимум ${fmt(q(ds, 1))}`);
