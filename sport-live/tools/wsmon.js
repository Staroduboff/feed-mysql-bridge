...
// Живёт ОТДЕЛЬНЫМ процессом (wsmon-feed.js) и только поэтому живёт вообще: пока
// подписка была здесь, рядом с браузерами, она отваливалась на шестой минуте и
// не возвращалась — 19.09.2026 так пропали 174 минуты из 180. Главный процесс
// секунду за секундой ждёт ответа Playwright, и сокету не хватало времени
// вычитывать поток и отвечать на пинги; та же подписка без браузеров держится
// без единого разрыва. Здесь остаётся только приём готовых дельт.
//
// Зачем она вообще: витрина подписывается по видимой области и маркеты событий
// за экраном не получает. Если брать истину из её кадров, монитор не отличит
// «фронт не показал» от «фронт и не знал», а сравнение витрин между собой
// теряет смысл (18.09 из-за этого совпадение упало с 98.85 до 93.57 % при
// неизменном фиде).
const own = { proc: null, channels: 0, pubs: 0, drops: 0, errors: 0,
  connected: false, downMs: 0, lastCode: null, lastReason: '', restarts: 0 };

function startOwnFeed() {
  const { fork } = require('child_process');
  const spawn = () => {
    own.proc = fork(__dirname + '/wsmon-feed.js', [], { stdio: ['ignore', 'ignore', 'ignore', 'ipc'] });
    own.proc.on('message', (m) => {
      if (!m) return;
      if (m.deltas) { m.deltas.forEach((p) => walkDeltas(p.d, p.t)); return; }
      if (m.stat) {
        own.channels = m.stat.channels; own.pubs = m.stat.pubs; own.drops = m.stat.drops;
        own.errors = m.stat.errors; own.connected = m.stat.connected; own.downMs = m.stat.downMs;
        own.lastCode = m.stat.lastCode; own.lastReason = m.stat.lastReason || '';
        return;
      }
      if (m.log) log('[feed]', m.log);
    });
    // Процесс подписки не должен уносить с собой замер: поднимаем заново.
    own.proc.on('exit', (code) => {
      own.connected = false;
      if (T0 && Date.now() - T0 < MIN * 60000) {
        own.restarts++;
        log('[feed] процесс подписки завершился (код ' + code + '), поднимаю заново');
...
function stopOwnFeed() {
  if (!own.proc) return;
  try { own.proc.removeAllListeners('exit'); own.proc.send('stop'); } catch (e) { /* уже ушёл */ }
  setTimeout(() => { try { own.proc.kill(); } catch (e) { /* уже ушёл */ } }, 1000);
}
...
function compare(s, dom, now) {
  ...
  // Пустой экран — не «нет расхождений», а сломанная страница. 20.09 витрина
  // молчала три часа, и отчёт об этом не сказал ни слова.
  if (dom.size === 0) {
    s.blank = (s.blank || 0) + 1;
    if (s.blank === 60) log('[' + (s.tag || s.name) + '] экран пуст уже минуту — страница не отрисовала ни одной ячейки');
  } else s.blank = 0;
  s.blankTotal = (s.blankTotal || 0) + (dom.size === 0 ? 1 : 0);
...
function siteReport(s) {
  ...
  if (s.name !== 'Пижама') {
    const down = Math.round(own.downMs / 1000);
    const span = Math.max(1, Math.round((Date.now() - T0) / 1000));
    lines.push(`Своя подписка монитора (отдельный процесс): каналов ${own.channels}, публикаций ${own.pubs}, `
      + `разрывов ${own.drops}, перезапусков процесса ${own.restarts}, без связи ${down} с из ${span}, ошибок ${own.errors}`
      + (own.lastCode != null ? ` (последний разрыв: ${own.lastCode} ${own.lastReason})` : ''));
    // Без этой строки прогон с мёртвой подпиской читается как обычный: истина
    // монитора замирает, а расхождения списываются на фронт (так вышло 19.09).
    if (down > span * 0.05) {
      lines.push(`    !! ЗАМЕР НЕДОСТОВЕРЕН: своя подписка молчала ${Math.round(down / span * 100)} % времени, `
        + `«наша линия» и сравнение витрин в этом отчёте считать нельзя`);
    }
  }
  // Отчёт обязан признаваться, что мерить было нечего: иначе пустая страница
  // читается как «расхождений нет».
  const blankShare = s.samples ? (s.blankTotal || 0) / s.samples : 0;
  if (blankShare > 0.2) {
    lines.push(`    !! ЗАМЕР НЕДОСТОВЕРЕН: экран был пуст в ${Math.round(blankShare * 100)} % опросов`
      + (s.pageErrs ? `, ошибок страницы ${s.pageErrs}` : ', ошибок страницы не было')
      + ' — сравнивать в этом отчёте нечего');
  } else if (s.pageErrs) {
    lines.push(`    ошибок страницы за прогон: ${s.pageErrs} (первые десять — в журнале)`);
  }
...
(async () => {
  ...
  // Ошибки страниц не собирались вовсе, поэтому 20.09 осталось неизвестным, на
  // чём именно споткнулась витрина: в журнале не было ни строчки.
  const watchPage = (page, s) => {
    const note = (kind, text) => {
      s.pageErrs = (s.pageErrs || 0) + 1;
      if (s.pageErrs <= 10) log('[' + s.tag + '] ' + kind + ':', String(text).slice(0, 200));
    };
    page.on('pageerror', (e) => note('ошибка страницы', e));
    page.on('console', (m) => { if (m.type() === 'error') note('консоль', m.text()); });
    page.on('requestfailed', (r) => {
      const u = r.url();
      if (/\.(js|json)(\?|$)/.test(u)) note('запрос не удался', u.slice(-70) + ' ' + ((r.failure() || {}).errorText || ''));
    });
  };
  S.our.tag = 'our'; S.pz.tag = 'pz';
  watchPage(our, S.our); watchPage(pz, S.pz);
  ...
  // Своя подписка держит таймер и сокет, поэтому без явной остановки процесс
  // не завершается и продолжает тянуть весь фид — 19.09 так провисел лишний
  // монитор десять часов и едва не столкнулся со следующим прогоном.
  stopOwnFeed();
  await bOur.close(); await bPz.close();
  process.exit(0);   // подстраховка: сокет своей подписки может пережить disconnect()
