...
print("\n10. Потерянная дельта закрывает приём ставок")
# Замер 14.09.2026: под нагрузкой публикатор получал таймаут на /api/batch и выбрасывал
# дельты — цена на экране игрока оставалась прежней, хотя в базе уже изменилась. Такая
# ставка оплачивается из нашего кармана, поэтому потеря закрывает приём независимо от
# того, что все остальные сигналы в норме.
g = fresh_gate()
t = warm(g, T)
check("разогрев открыт", g.state, STATE_OPEN)
g.note_drop(37, now=t)                     # публикатор не доставил 37 дельт
g._uts_cur = uts(t)
g._last_msg = t
g.tick(now=t)
check("потеря закрывает приём", g.state, STATE_SUSPEND)
check("названа верная причина", g.reasons, ["pubdrop"])
check("счётчик потерь виден", g.metrics["pdrops"], 37)
# приём не открывается, пока не истечёт окно, и потом ещё выдержка
t += int(g.thr["pub_drop_window"]) - 5
g._uts_cur = uts(t); g._last_msg = t; g.tick(now=t)
check("внутри окна всё ещё закрыт", g.state, STATE_SUSPEND)
for _ in range(int(g.thr["reopen_hold"]) + 12):
    t += 1
    g._uts_cur = uts(t); g._last_msg = t; g.tick(now=t)
check("после окна и выдержки открылся", g.state, STATE_OPEN)

print("\n11. Тик идёт из фонового потока, без внешних вызовов")
